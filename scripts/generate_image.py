...
def get_api_key():
    ...
    Checks GEMINI_API_KEY environment variable first.
    If not found, falls back to the hardcoded value (not recommended).
    
    Get your API key from: https://aistudio.google.com/apikey
    """
    # 1. Check environment variable (Recommended)
...
def generate_image(
    ...
    if api_key == "YOUR_GEMINI_API_KEY_HERE" or not api_key:
        ...
        print("\nOr edit scripts/generate_image.py and replace YOUR_GEMINI_API_KEY_HERE (not recommended).", file=sys.stderr)
        sys.exit(1)
...
Examples:
  ...
  %(prog)s "Combine styles" --ref ./img1.png --ref ./img2.png -o ./combined.png
    ...
    parser.add_argument(
        "--output", "-o",
        required=False,
        help="Output file path. If omitted, saves to './generated/' with auto-generated name.",
    )
    parser.add_argument(
        "--aspect", "-a",
    ...
    parser.add_argument(
        ...
        help="Model to use: 'pro' (higher quality, follow instructions) or 'flash' (faster speed). Default: pro",
    )
    parser.add_argument(
        "--key", "-k",
        help="Gemini API Key (optional, overrides environment variable)",
    )
    parser.add_argument(
        "--ref", "-r",
        action="append",
    ...
    print(f"Generating image with {model_id}...", file=sys.stderr)
    print(f"Prompt: {args.prompt}", file=sys.stderr)
    print(f"Aspect: {args.aspect}, Size: {args.size}, Model: {args.model}", file=sys.stderr)
    ...
    if args.output:
        ...
        # Auto-generate filename: generated/slug-timestamp.ext
        import time
        import re
        slug = re.sub(r'[^\w\s-]', '', args.prompt).strip().lower()
        slug = re.sub(r'[-\s]+', '-', slug)[:50] or "untitled"
        timestamp = int(time.time())
        filename = f"{slug}-{timestamp}" # extension added below
        output_path = Path("generated") / filename

    correct_ext = get_extension(mime_type)

    # Replace any user-specified extension with the correct one (or add it)
